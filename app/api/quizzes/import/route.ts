...
const OPTIONAL_COLUMNS = ["level", "option2", "option3", "option4", "answer", "explaining"];
    ...
    try {
        ...
        for (let f = 0; f < files.length; f++) {
            ...
            try {
                ...
                const expectedHeaders = [...REQUIRED_COLUMNS, ...OPTIONAL_COLUMNS].map((h) => h.toLowerCase().trim());

                // Check for missing required columns
                const missingCols = REQUIRED_COLUMNS.filter((col) => !headers.includes(col));
                if (missingCols.length > 0) {
                    allErrors.push(
                        `File ${fileName}: Missing required columns: ${missingCols.join(", ")}. Required: ${REQUIRED_COLUMNS.join(", ")}. Optional: ${OPTIONAL_COLUMNS.join(", ")}`
                    );
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    if (!category || !topic || !question) {
                        fileErrors.push(
                            `File ${fileName} - Row ${rowNum}: (category: ${category} - topic: ${topic} - question: ${question}) missing required fields`
                        );
                        continue;
                    }
                    ...
                    if (options.length === 0) {
                        fileErrors.push(
                            `File ${fileName} - Row ${rowNum}: (category: ${category} - topic: ${topic} - question: ${question}) at least one option (option1) is required`
                        );
                        continue;
                    }

                    const ansRaw = Number(row.answer);
                    const answer = !isNaN(ansRaw) && ansRaw >= 0 && ansRaw < options.length ? ansRaw : 0;
                    ...
                    validDocs.push({
                        ...
                        options,
                        answer,
                        explaining: String(row.explaining ?? "").trim(),
                        createdAt: new Date(),
                        updatedAt: new Date(),
                    });
                }
